...
`avatar` is a signed URL from `EventAttendee::avatarUrl()`, serialised in five places: the Attendee list and detail, the Standings, a Round's pairings and a Game. It is drawn at `lg` on the Attendee's own page and on the team's details screen, `xs` in a pairing, `sm` in the Attendees list and the Standings, where it stands beside the whole two-line cell rather than beside the name alone. Null is the common answer, and every screen draws `TeamAvatar`'s initials placeholder rather than a gap.
