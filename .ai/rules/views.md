...
The Attendee screen carries no back link: the Attendees tab is pinned one tap away and leads to the same place. The Round screen carries none either, for a stronger reason — it *is* what the Rounds tab reaches, so there is nothing behind it to go back to. The Poll screen keeps its back link, because the nav does not reach the Votes list. The organiser screens keep theirs too, even though the Organisers tab now reaches the hub they hang off. Do not add a back link to a screen whose parent is a nav tab.
